...
public class QuicksortIntDecrescente {
    public static void main(String[] args) {
        ...
        quicksortIntDecrescente(vetor, 0, vetor.length - 1);  // CORRIGIDO
    ...
    // PARA FAZER DECRESCENTE, BASTA MUDAR O VALOR DO > PARA < E VICE VERSA
    ...
    public static void quicksortIntDecrescente(int vetor[], int inicio, int fim) {
        int particao;

        if (fim > inicio) {  // tamanho do segmento é maior que 1 ?
            particao = particao(vetor, inicio, fim);
            quicksortIntDecrescente(vetor, inicio, particao - 1);
            quicksortIntDecrescente(vetor, particao + 1, fim);
        }
    }
}
